...
def adddoctor():
    if request.method=='POST':
        ...
        if data == []:
            ...
            sender_address = 'chaitramallikarjun0@gmail.com'
            sender_pass = 'rvsvtwjfrmjyhoww'
            content='The admin has added The doctor : email: '+ email + ' and password '+ password +' '
            receiver_address = email
            message = MIMEMultipart()
            message['From'] = sender_address
            message['To'] = receiver_address
            message['Subject'] = 'MEDICAL REPORT MANAGEMENT & DISTRIBUTION SYSTEM ON BLOCK CHAIN'
...
def receptionist():
    if request.method=='POST':
        ...
        if data ==[]:
            ...
            sender_address = 'chaitramallikarjun0@gmail.com'
            sender_pass = 'rvsvtwjfrmjyhoww'
            content='The admin has added The RECEPTIONIST : email: '+ email + ' and password '+ password +' '
            receiver_address = email
            message = MIMEMultipart()
            message['From'] = sender_address
            message['To'] = receiver_address
...
def addpatients():
    if request.method=='POST':
        ...
        sender_address = 'chaitramallikarjun0@gmail.com'
        sender_pass = 'rvsvtwjfrmjyhoww'
        content='The Patient appointment has conformed  : email: '+ email + ' and password '+ password +' '
        receiver_address = email
        message = MIMEMultipart()
        message['From'] = sender_address
        message['To'] = receiver_address
...
def viewfinalreports(r=0,r1='',r2=''):
    print(r,r1,r2)
    sql="select Id,Name,Symptoms,StartDate,Filedata,EndDate,HashA,HashB from reports where Slno='%s' or Name='%s' and Symptoms='%s'"%(r,r1,r2)
    cur.execute(sql)
    data=cur.fetchall()
    db.commit()
    print(data)
    ...
    sender_address = 'chaitramallikarjun0@gmail.com'
    sender_pass = 'rvsvtwjfrmjyhoww'
    content='The Hash Key to Decrypt the key : Hash1: '+ HashA + ' and Hash2 '+ HashB +' '
    receiver_address = Email
    message = MIMEMultipart()
    message['From'] = sender_address
    message['To'] = receiver_address
...
def getdata():
    if request.method=='POST':
        ...
        print(hasha,hashb,session['patient'])
